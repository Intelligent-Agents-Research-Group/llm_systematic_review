...
DEFAULT_MODEL_DIR = "unsloth/LFM2.5-1.2B-Instruct"  # "lfm-1.2_for_sys_review"
...
# Output paths
EVAL_RESULTS_PATH = "results/eval_results_full_dataset_unsloth_no_finetune.json"
SKIPPED_EXAMPLES_PATH = "results/skipped_examples_full_dataset_unsloth_no_finetune.jsonl"
DETAILED_LOG_PATH = "results/evaluation_detailed_log_full_dataset_unsloth_no_finetune.jsonl"
...
def compute_classification_metrics(labels: list, preds: list) -> dict:
    ...
    Returns dict with: accuracy, precision, recall, f1, tp, tn, fp, fn, total
    """
    ...
    if total == 0:
        return {
            "total": 0, "accuracy": 0.0, "precision": 0.0, "recall": 0.0,
...
def evaluate_model(
    ...
    total_to_process = max_samples if max_samples else total_rows
    
    # Process each example with tqdm progress bar
    for idx, row in enumerate(tqdm(dataset, total=total_to_process, desc="Evaluating", unit="sample")):
